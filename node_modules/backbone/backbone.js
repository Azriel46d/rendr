...
(function(){
 ...
 //TODO SOCKET.IO
     ...
     * @name sync
     */
    Backbone._ioListeners = {
        'get' :{},
        'post' :{},
        'put' :{},
        'delete':{}
    }
    Backbone.addIoListener = function(key,method,options,defer){


        if (key && method && options){

            Backbone._ioListeners[method][key] = {
                options : options,
                defer : defer
            };
        }
    ...
    Backbone.removeIoListener = function(key,method){
        try {
            delete Backbone._ioListeners[method][key]
        } catch(e){}
    };
    Backbone.getIoListener = function(key,method){
        return Backbone._ioListeners[method][key] || null;
    }

    Backbone.sync = function (method, model, options) {
        var type = methodMap[method].toLowerCase(); //GET | POST | PUT | Delete
        ...
        var io = model && model.socket || Backbone && Backbone.socket || window.socket

        var defer = $.Deferred();
        var io_key = _.uniqueId('io_');
        //since Backbone version 1.0.0 all events are raised in methods 'fetch', 'save', 'remove' etc

        //register callbacks
        Backbone.addIoListener(io_key,type,options,defer);
        ...
        //trigger model's request with promise
